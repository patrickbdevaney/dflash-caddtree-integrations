...
class MambaModelConfig(VerifyAndUpdateConfig):
    ...
    def verify_and_update_config(cls, vllm_config: "VllmConfig") -> None:
        ...
        if cache_config.enable_prefix_caching:
            ...
            # DECOUPLING (GDN-APC gate fix): mamba_cache_mode was previously forced
            # to 'none' whenever prefix caching is disabled, making it impossible to
            # run a COLD baseline in 'align' mode for the cold==warm correctness gate.
            # The APC flag should control whether the cache is USED, not which GDN
            # compute path runs (none=contiguous vs align=block-aligned) -- these are
            # orthogonal. Change: if the user EXPLICITLY set 'align', honor it even
            # with APC off (cold-align baseline). Default ('none') is unchanged for
            # every config that does not explicitly request align.
            if cache_config.mamba_cache_mode == "align":
                assert vllm_config.scheduler_config.enable_chunked_prefill, (
                    "Chunked prefill is required for mamba cache mode 'align'."
                )
