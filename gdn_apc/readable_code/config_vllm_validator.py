...
class VllmConfig:
    ...
    def validate_mamba_block_size(self) -> "VllmConfig":
        ...
        # DECOUPLING (GDN-APC gate fix): also allow explicit mamba_block_size in
        # 'align' mode with APC off, so a COLD align baseline uses the SAME align
        # block size as the WARM (APC-on) run -> the cold==warm gate isolates ONLY
        # the cache. Default rejection preserved for non-align configs without APC.
        if (
            mamba_block_size_is_set
            and not self.cache_config.enable_prefix_caching
